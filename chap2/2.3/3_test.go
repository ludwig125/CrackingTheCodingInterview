...
func TestSingleLinkedListDeleteTargetNode(t *testing.T) {
	tests := map[string]struct {
		...
		"0": {
			list:       []int{10, 11, 12},
			targetNode: 2, // 1始まりなので11を消す
			want:       newSingleLinkedList([]int{10, 12}...),
		},
		"1": {
			list:       []int{10, 11, 12, 13},
			targetNode: 2, // 1始まりなので11を消す
			want:       newSingleLinkedList([]int{10, 12, 13}...),
		},
		"2": {
			list:       []int{10, 11, 12, 13},
			targetNode: 3, // 1始まりなので12を消す
			want:       newSingleLinkedList([]int{10, 11, 13}...),
		},
	}
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			l.print()
			l.deleteTargetNode(tt.targetNode)
			// fmt.Println("value", l.getValues())
...
func (s *singleLinkedList) deleteTargetNode(targetNode int) {
	cnt := 0
	s1 := s
	for {
		cnt++
		// fmt.Println("v", s1.getValues())
		if s1.isLast() {
			break
		}
		// fmt.Println("cnt", cnt, s1.getValues(), targetNode)
		if cnt == targetNode {
			*s1 = singleLinkedList{value: s1.next.value, next: s1.next.next}
			break
		}
